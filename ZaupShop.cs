...
namespace ZaupShop
{
    public class ZaupShop : RocketPlugin<ZaupShopConfiguration>
    {
        ...
        public bool Buy(UnturnedPlayer playerid, string[] arguments)
        {
            string message;
            ...
            {
                if (!byte.TryParse(arguments[1], out amttobuy))
                {
            ...
            if ((components.Length == 2 && components[0].Trim() != "v") || (components.Length == 1 && components[0].Trim() == "v") || components.Length > 2 || arguments[0].Trim() == string.Empty)
            {
                message = Instance.Translate("buy_command_usage");
                // We are going to print how to use
                UnturnedChat.Say(playerid, message);
            ...
            switch (components[0])
            {
                case "v":
                    ...
                    if (balance < cost)
                    {
                        message = Instance.Translate("not_enough_currency_msg", Uconomy.Instance.Configuration.Instance.MoneyName, "1", name);
                        UnturnedChat.Say(playerid, message);
                        return false;
                    }
                    ...
                    message = Instance.Translate("vehicle_buy_msg", name, cost, Uconomy.Instance.Configuration.Instance.MoneyName, newbal, Uconomy.Instance.Configuration.Instance.MoneyName);
                    if (Instance.OnShopBuy != null)
                        Instance.OnShopBuy(playerid, cost, 1, id, "vehicle");
                ...
                default:
                    ...
                    if (balance < cost)
                    {
                        message = Instance.Translate("not_enough_currency_msg", Uconomy.Instance.Configuration.Instance.MoneyName, amttobuy, name);
                        UnturnedChat.Say(playerid, message);
                        return false;
                    }
                    playerid.GiveItem(id, amttobuy);
                    newbal = Uconomy.Instance.Database.IncreaseBalance(playerid.CSteamID.ToString(), (cost * -1));
                    message = Instance.Translate("item_buy_msg", name, cost, Uconomy.Instance.Configuration.Instance.MoneyName, newbal, Uconomy.Instance.Configuration.Instance.MoneyName, amttobuy);
                    if (Instance.OnShopBuy != null)
                        Instance.OnShopBuy(playerid, cost, amttobuy, id);
        ...
        public void Cost(UnturnedPlayer playerid, string[] arguments)
        {
            ...
            switch (components[0])
            {
                ...
                default:
                    ...
                    decimal bbp = Instance.ShopDB.GetItemBuyPrice(id);
                    message = Instance.Translate("item_cost_msg", name, cost.ToString(), Uconomy.Instance.Configuration.Instance.MoneyName, bbp.ToString(), Uconomy.Instance.Configuration.Instance.MoneyName);
                    if (cost <= 0m)
                    {
        ...
        public bool Sell(UnturnedPlayer playerid, string[] components)
        {
            ...
            decimal price = Instance.ShopDB.GetItemBuyPrice(id);
            if (price <= 0.00m)
            {
            ...
            message = Instance.Translate("sold_items", amt, name, addmoney, Uconomy.Instance.Configuration.Instance.MoneyName, balance, Uconomy.Instance.Configuration.Instance.MoneyName);
            if (Instance.OnShopSell != null)
                Instance.OnShopSell(playerid, addmoney, amt, id);
